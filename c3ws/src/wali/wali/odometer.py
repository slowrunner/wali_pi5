...
"""
   ...
   Subscribes: /odom, /stop_status (1Hz)

   Starts a recording a segment when stop_status.is_stopped=false
   (using the /odom from 1 second prior)
   Continues adding distance traveled at 1hz until stop_status.is_stopped=true
...
class OdometerNode(Node):
  ...
  def __init__(self):
    ...
    self.sub_stop_status = self.create_subscription(
      StopStatus,
      'stop_status',
      self.sub_stop_status_callback,
      qos_profile_sensor_data)
    if DEBUG: self.get_logger().info('stop_status topic subscriber created')
  ...
  def sub_stop_status_callback(self,stop_status_msg):

    self.stop_status_msg = stop_status_msg
    ...
    if DEBUG: self.get_logger().info('is_stopped: {}'.format(self.stop_status_msg.is_stopped))
    ...
    """
    if self.stop_status_msg.is_stopped == True:
